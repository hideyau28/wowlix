/**
 * 私隱政策正文 —— 平台 route（`app/[locale]/platform/privacy`）同租戶 route
 * （`app/[locale]/(customer)/privacy`）共用同一份 render。
 *
 * ⚠️ 抽出嚟**淨係為咗拆 route 邊界**（斷開 marketing font manifest），內容
 * 一個字冇改過。
 *
 * ⚠️ 住喺 `components/legal/`（唔係 `components/marketing/`）：租戶 route 都
 * import 呢個檔，一 pull 到 marketing 嘢就會將 Fraunces 綁返落租戶 font
 * manifest。**呢個檔淨係准 import 純邏輯 util。**
 */
// 註：正文由頭到尾冇 interpolate 店名（原本個 page 亦然），所以淨收 isZh。
export default function PrivacyBody({ isZh }: { isZh: boolean }) {
  if (isZh) {
    return (
